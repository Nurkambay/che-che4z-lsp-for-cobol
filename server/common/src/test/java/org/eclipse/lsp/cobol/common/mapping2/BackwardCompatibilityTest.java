...
import lombok.var;
import org.eclipse.lsp.cobol.common.mapping.MappingService;
...
class BackwardCompatibilityTest {
  ...
  void testMappingWithErasingForRange1() {
    ExtendedText textTransformations = new ExtendedText(TEXT_REPLACEMENT_FOR_RANGE, "original");
    Range copybookRange = new Range(new Position(1, 2), new Position(3, 5));

    textTransformations.replace(copybookRange, "");
    copybookRange = new Range(new Position(4, 2), new Position(6, 5));

    textTransformations.insert(copybookRange, new ExtendedText(BIG_COPY, "copybook2"));

    Range copyRange = new Range(new Position(2, 2), new Position(2, 4));
    Location originalLocation1 = textTransformations.mapLocation(copyRange);

    Range bottomRange = new Range(new Position(17, 2), new Position(8, 4));
    Location originalLocation2 = textTransformations.mapLocation(bottomRange);

    assertNotNull(originalLocation1);
    assertEquals(0, originalLocation1.getRange().getStart().getLine());
    assertEquals(2, originalLocation1.getRange().getStart().getCharacter());

    assertNotNull(originalLocation2);
    assertEquals(7, originalLocation2.getRange().getStart().getLine());
    assertEquals(2, originalLocation2.getRange().getStart().getCharacter());
  }
